...
public class get_arc {
    ...
    public void get_circle(MouseEvent e)
{	
		...
		{
			s2 = e.getPoint();
                        if(SCR.a3.calc_distance(s2.x,s2.y,s1.x,s1.y)>5){
			...
			s3 = e.getPoint();
			if((SCR.a3.calc_distance(s2.x,s2.y,s3.x,s3.y)>5)&&(SCR.a3.calc_distance(s3.x,s3.y,s1.x,s1.y)>5)
                                &&!SCR.a3.collinear(s1.x,s1.y,s2.x,s2.y,s3.x,s3.y)){
                    System.out.println("Reached :- "+ s1+" , "+s2+","+s3);
